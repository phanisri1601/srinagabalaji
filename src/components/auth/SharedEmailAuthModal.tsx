...
interface SharedEmailAuthModalProps {
  isOpen: boolean;
  onClose: () => void;
  onAuthSuccess: (user: { name: string; phone: string; uid: string }) => void;
}

export const SharedEmailAuthModal: React.FC<SharedEmailAuthModalProps> = ({
  isOpen,
  onClose,
  onAuthSuccess
}) => {
  ...
  // Shared email account for all users
  const SHARED_EMAIL = 'nagabalaji@demo.com';
  ...
  const handleEmailSubmit = async () => {
    ...
    try {
      ...
      // Generate demo OTP for now (in production, this would be sent via email)
      const demoOtp = '123456';
      
      console.log('Creating account for:', email);
      console.log('Demo OTP:', demoOtp);
      console.log('In production: OTP would be sent to user\'s email address');
      
      // Create Firebase Auth account with user's email
  ...
  const handleVerifyOTP = async () => {
    ...
    try {
      ...
      // Sign in with shared email account
      const result = await signInWithEmailAndPassword(auth, SHARED_EMAIL, SHARED_PASSWORD);
      
      if (result.user) {
        // OTP verified, create or update user in our database
        const existingUser = await userService.getUserByPhone(phoneNumber);
            ...
            <div className="bg-white rounded-2xl shadow-2xl max-w-md w-full max-h-[90vh] overflow-y-auto">
              ...
              <div className="flex items-center justify-between p-6 border-b border-emerald-100 bg-gradient-to-r from-emerald-50 to-rose-50">
                <div className="flex items-center space-x-3">
                  ...
                  <div>
                    <h2 className="text-xl font-bold text-gray-800">Shared Login</h2>
                  ...
                  >
                    <div className="text-center mb-6">
                      ...
                      <p className="text-xs text-amber-600 font-medium">
                        📧 Using shared account: nagabalaji@demo.com
                      </p>
                    </div>
                ...
                {step === 'email' && (
                  ...
                  >
                    <div className="text-center mb-6">
                      ...
                      <p className="text-xs text-amber-600 font-medium">
                        📧 We'll create your account with this email
                      </p>
                    </div>
                ...
                {step === 'phone' && (
                  ...
                  >
                    <div className="text-center mb-6">
                      ...
                      <p className="text-xs text-amber-600 font-medium">
                        📱 We'll check for existing account
                      </p>
                    </div>
                  ...
                  >
                    <div className="text-center mb-6">
                      ...
                      <p className="text-gray-600 mb-2">
                        OTP sent to {SHARED_EMAIL}
                      </p>
                      <p className="text-sm text-gray-500">
                        Demo OTP: {SHARED_PASSWORD}
                      </p>
                    </div>
